...
(function runDeckCountTests() {
    ...
    // `initFirstTurn` compra mais 1 para quem abre (p1). O rótulo tem de bater
    // com o saldo do estado em qualquer um dos dois decks.
    ...
    assert('a soma mão + deck continua 50 nas duas áreas',
        saldo('p1') + mao('p1') === 50 && saldo('p2') + mao('p2') === 50);
    ...
    window.resetGame();
    assert('nova partida devolve o saldo ao rótulo (e a soma volta a 50)',
        texto('p1') === `${saldo('p1')} cartas` && saldo('p1') + mao('p1') === 50
        && texto('p2') === `${saldo('p2')} cartas` && saldo('p2') + mao('p2') === 50);
